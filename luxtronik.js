...
function parseFirmware(buf) {
    var firmware = ''
    for (i in buf) {
        firmware += (buf[i] == 0) ? '' : String.fromCharCode(buf[i]);
    }
    return firmware;
}
...
function processData() {
    var payload = {};
    const heatpump_parameters = toInt32ArrayReadBE(receivy['3003'].payload);
    const heatpump_values = toInt32ArrayReadBE(receivy['3004'].payload);
    const heatpump_visibility = receivy['3005'].payload;
        ...
        if (receivy.rawdata) {
            ...
            payload = {
                values: {
                    ...
                    'errors': [
                        new Date(heatpump_values[95] * 1000).toString() + ' - ' + heatpump_values[100], // #42
                        new Date(heatpump_values[96] * 1000).toString() + ' - ' + heatpump_values[101],
                        new Date(heatpump_values[97] * 1000).toString() + ' - ' + heatpump_values[102],
                        new Date(heatpump_values[98] * 1000).toString() + ' - ' + heatpump_values[103],
                        new Date(heatpump_values[99] * 1000).toString() + ' - ' + heatpump_values[104]
                    ],

                    'error_count': heatpump_values[105],

                    'switch_off': [
                        new Date(heatpump_values[111] * 1000).toString() + ' - ' + heatpump_values[106],
                        new Date(heatpump_values[112] * 1000).toString() + ' - ' + heatpump_values[107],
                        new Date(heatpump_values[113] * 1000).toString() + ' - ' + heatpump_values[108],
                        new Date(heatpump_values[114] * 1000).toString() + ' - ' + heatpump_values[109],
                        new Date(heatpump_values[115] * 1000).toString() + ' - ' + heatpump_values[110]
                    ],
                ...
                additional: {
                    'reading_calculated_time': receivy.readingEndTime - receivy.readingStartTime,
                }
            };
...
function startWrite(host, port, parameterName, realValue) {
    ...
    if (parameterName === 'heating_target_temperature') {
        ...
        setValue = parseInt(realValue * 2) * 5;
